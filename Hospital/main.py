...
args.log_path = os.path.join(args.log_base, args.set, f'S_{args.scales}',  f'seed_{args.seed}')
...
if args.set == 'no_overlap':
    args.data_dir = f"./data/S_{args.scales}/non_overlap"
...
def main():
    ...
    for epoch in loop:
        ...
        if args.save_model:
            args.model_save_dir = f'models/{args.scales}'
...
def plot():
    class_mapping = {'s1': 0, 's2': 1, 's3': 2, 's8': 3, 's13': 4, 's5': 5}
    inverse_class_mapping = {v: k for k, v in class_mapping.items()}
    model_path = 'models/64/model_epoch_48.pt'  # 替换为实际的模型文件路径
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # data = data.permute(0,4,2,3,1).squeeze(1)
    ...
    for class_id in range(num_classes):
        ...
        plt.scatter(features_tsne[indices, 0], features_tsne[indices, 1], 
                    label=f'{inverse_class_mapping[class_id]}', alpha=0.7)

    plt.legend()
    plt.title('t-SNE of Hospital')
    plt.tight_layout()
