...
def main():
    ...
    for epoch in loop:
        ...
        test_loss, test_acc, f1_weighted, roc_auc, kappa, precision, recall = evaluate(model, test_loader, criterion)
        ...
        logging.info(f'\tWeigthed F1:{f1_weighted:.4f}| ROC score:{roc_auc:.2f} ')
...
def evaluate(model, iterator, criterion):
    ...
    with torch.no_grad():
        ...
        print(cm)
        # 计算加权F1得分
        f1_weighted = f1_score(np.argmax(cm, axis=1), np.argmax(cm, axis=1), average='weighted')
        ...
        return losses.avg, accuracies.avg, f1_weighted, roc_auc, kappa, precision, recall
